    ...
    grunt.registerTask('removeCopies', function () {
        ...
        for (var i = 0; i < dir.length; i++) {
            ...
            for (var j = 0; j < subdir.length; j++) {
                // ic_add_24px.svg
                // ic_add_48px.svg
                if (subdir[j].indexOf('24dp.') != -1) {
                    if (fs.existsSync(__dirname + '/www/' + dir[i] + '/' + subdir[j].replace('24dp.', '48dp.'))) {
                        fs.unlinkSync(__dirname + '/www/' + dir[i] + '/' + subdir[j]);
                    }
                } else if (subdir[j].indexOf('18dp.') != -1) {
                    if (fs.existsSync(__dirname + '/www/' + dir[i] + '/' + subdir[j].replace('18dp.', '48dp.'))) {
                        fs.unlinkSync(__dirname + '/www/' + dir[i] + '/' + subdir[j]);
                    }
                }else if (subdir[j].indexOf('36dp.') != -1) {
                    if (fs.existsSync(__dirname + '/www/' + dir[i] + '/' + subdir[j].replace('36dp.', '48dp.'))) {
                        fs.unlinkSync(__dirname + '/www/' + dir[i] + '/' + subdir[j]);
                    }
                }
            }
